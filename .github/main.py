...
def main(page: ft.Page):
    page.title = "Système Solo Leveling"
    page.bgcolor = "#0B0C10"
    ...
    def xp_necessaire_pour_niveau(niveau):
        ...
        else: return math.ceil((niveau * base_xp) * 2.0)

    def calculer_xp_multi_plateaux(quete, streak_cible):
        base_xp = quete.get("base_xp", 10)
        paliers = quete.get("paliers", [[10, 1.05], [25, 1.01], [999, 1.005]])
    ...
    def parser_paliers_texte(texte):
        paliers = []
        if not texte.strip():
            return [[10, 1.05], [25, 1.01], [999, 1.005]]
        try:
            blocs = texte.split(",")
            for b in blocs:
                if ":" in b:
            ...
            return paliers if paliers else [[10, 1.05], [25, 1.01], [999, 1.005]]
        except Exception:
            return [[10, 1.05], [25, 1.01], [999, 1.005]]
    ...
    def rafraichir_header_xp():
        ...
        if niv < 5: titre_rang = "E-RANK"
        elif niv < 10: titre_rang = "D-RANK"
        elif niv < 15: titre_rang = "C-RANK"
        else: titre_rang = "B-RANK"
            
        lbl_niveau.value = f"{titre_rang} | NIVEAU {niv}"
        lbl_xp_texte.value = f"{xp_actuel} / {xp_max} XP"
        page.update()
    ...
    dropdown_jours_cible = ft.Dropdown(
        ...
        border_color="#4B0082",
        options=[
            ft.dropdown.Option("1", "1 fois par semaine"),
            ft.dropdown.Option("2", "2 fois par semaine"),
            ft.dropdown.Option("3", "3 fois par semaine"),
            ft.dropdown.Option("4", "4 fois par semaine"),
            ft.dropdown.Option("5", "5 fois par semaine"),
            ft.dropdown.Option("6", "6 fois par semaine"),
            ft.dropdown.Option("7", "7 fois par semaine (Quotidienne)"),
        ],
        value="7"
    )

    txt_xp = ft.TextField(label="XP de base (ex: 10)", value="10", border_color="#4B0082", keyboard_type=ft.KeyboardType.NUMBER)
    text_paliers = ft.TextField(label="Plateaux Multiples (Format: jour:facteur)", value="10:1.05, 25:1.01, 999:1.005", border_color="#4B0082", focused_border_color="#00FFFF")
    ...
    def ajouter_nouvelle_quete(e):
        ...
        # Correction : On nettoie et on met à jour la page immédiatement avant la transition
        txt_titre.value = ""
        dropdown_jours_cible.value = "7"
        txt_xp.value = "10"
        text_paliers.value = "10:1.05, 25:1.01, 999:1.005"
        page.update() 
        
        # On rebascule proprement sur l'onglet Liste
    ...
    btn_onglet_liste = ft.ElevatedButton(
        content=ft.Row([ft.Icon(ft.Icons.LIST_ALT, color="#00FFFF", size=18), ft.Text("Quêtes", color="#FFFFFF", size=12)]),
        bgcolor="#1A1A2E",
    ...
    btn_onglet_creer = ft.ElevatedButton(
        content=ft.Row([ft.Icon(ft.Icons.ADD_BOX, color="#00FFFF", size=18), ft.Text("Créer", color="#FFFFFF", size=12)]),
        bgcolor="#0B0C10",
        on_click=changer_onglet_vers_creer,
        expand=True
    )

    barre_onglets = ft.Row(controls=[btn_onglet_liste, btn_onglet_creer], alignment=ft.MainAxisAlignment.SPACE_EVENLY)
